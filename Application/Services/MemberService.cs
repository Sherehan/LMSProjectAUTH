...
namespace LMSProjectAUTH.Application.Services
{
    public class MemberService
    {
        ...
        public MemberStatisticVM GetMemberStatisicDB(int? memberId)
        {
            MemberStatisticVM model = new MemberStatisticVM
            {
                TotalBorrowsBookTillNow= _dbContext.Borrows.Count(m => m.MemberId == memberId),
                TotalBorrowedBooks = _dbContext.Borrows.Count(d => d.ReturnDate == null && d.MemberId == memberId),
                DelayedBook= _dbContext.Borrows.Count(x => x.ReturnDate == null &&
